...
--     "dismissed_discovery_menu":       boolean,                    -- first-run red dot on the mobile Menu icon
--     "dismissed_discovery_menu_at":    ISO timestamp string,
--     "dismissed_discovery_people":     boolean,                    -- first-run red dot on the People sidebar entry
...
-- Two legacy names exist in the wild: the original `activities_recurrence_type_check`
-- (default Postgres autogenerated name for a CHECK on column recurrence_type) and
-- the later explicit `activities_recurrence_check`. Drop both before recreating.
alter table public.activities drop constraint if exists activities_recurrence_type_check;
...
alter table public.notifications
  ...
  check (type in (
    ...
    'activity_reminder', 'reminder', 'activity_comment'
  ));
